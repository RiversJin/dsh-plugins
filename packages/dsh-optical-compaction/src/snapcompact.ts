...
/**
 * Faithful TypeScript port of @oh-my-pi/snapcompact 18.0.10, upstream commit
 * 33cc6b9a043a74e00a157e72ca909272796d8461. Host-specific imports are kept
 * behind this small compatibility vocabulary; the archive, shape, layout,
 * normalization, rendering, and foveation algorithms below remain upstream.
 */
...
/** Estimate how many model-context tokens one raster image occupies. The
 * provider formulas are the same live-usage-calibrated rules used by OMP's
 * context budgeting, extended to height-hugging rectangular frames. Unknown
 * APIs retain the conservative Anthropic-style ceiling. */
export function estimateImageContextTokens(api: Api | undefined, width: number, height: number): number {
	if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
		throw new Error(`Invalid image dimensions for token estimation: ${width}x${height}`);
	}
	...
	switch (family) {
		case "google":
			return 1120;
		case "openai": {
			const patches = Math.min(Math.ceil(width / 32) * Math.ceil(height / 32), 10_000);
			return Math.ceil(patches * 1.2);
		}
		default: {
			const patches = Math.min(Math.ceil(width / 28) * Math.ceil(height / 28), 4784);
			return Math.ceil(patches * 1.05);
		}
	}
}
...
function familyBilling(family: BillingFamily, frameSize: number): Pick<Shape, "frameTokenEstimate" | "imageDetail"> {
	switch (family) {
		case "google":
			return { frameTokenEstimate: estimateImageContextTokens("google-generative-ai", frameSize, frameSize) };
		case "openai": {
			return { frameTokenEstimate: estimateImageContextTokens("openai-responses", frameSize, frameSize), imageDetail: "original" };
		}
		default: {
			return { frameTokenEstimate: estimateImageContextTokens(undefined, frameSize, frameSize) };
		}
	}
}
...
export function idealShapeVariant(modelId: string): IdealShape | undefined {
	// Inline pi-catalog's exact Claude classifier so the Node port preserves
	// both kind-first (`claude-opus-4-7`) and version-first
	// (`claude-4.7-opus`) forms without pulling in the Bun-oriented catalog.
	const normalized = modelId.toLowerCase().split("/").at(-1) ?? modelId.toLowerCase();
	const kindFirst = /claude-(opus|sonnet|fable|mythos)-(\d{1,2}(?:[.-]\d{1,2}){0,2})\b/.exec(normalized);
	const versionFirst = kindFirst
		? null
		: /claude-(\d{1,2}(?:[.-]\d{1,2}){0,2})-(opus|sonnet|fable|mythos)\b/.exec(normalized);
	const kind = kindFirst?.[1] ?? versionFirst?.[2];
	const version = (kindFirst?.[2] ?? versionFirst?.[1])?.split(/[.-]/).map(Number);
	const major = version?.[0] ?? 0;
	const minor = version?.[1] ?? 0;
	const highResolutionClaude =
		kind === "fable" || kind === "mythos" || (kind === "opus" && (major > 4 || (major === 4 && minor >= 7)));
	if (highResolutionClaude) {
...
export interface Frame {
	...
	/** Time-bucket age when this frame was last rendered. */
	memoryAgePeriods?: number;
	/** Resolution multiplier applied for memory decay (1 = full resolution). */
	resolutionScale?: number;
	/** Absolute time bucket represented by this frame. */
	memoryBucketStart?: number;
}

/** One chronological source segment retained for timestamp-aware re-rendering. */
export interface TimedArchiveSegment {
	/** Start of the segment's absolute decay bucket, Unix epoch milliseconds. */
	bucketStart: number;
	/** Newest source event in this bucket, used to avoid aging newer content early. */
	latestAt: number;
	/** Normalized serialized transcript within that bucket. */
	text: string;
}
...
export interface Archive {
	...
	/** Cumulative effective source tokens represented or deliberately forgotten
	 *  by this archive. This keeps dynamic sizing stable across re-compaction: a
	 *  visual checkpoint's smaller replay bill must not masquerade as less
	 *  original information on the next pass. */
	...
	textTail?: string;
	/** Timestamped source used by the lazy sliding resolution-decay window. */
	segments?: TimedArchiveSegment[];
	/** Time of the render that produced the current frame set. */
	capturedAt?: number;
}
...
export interface Options<TMessage = Message> extends SerializeOptions {
	...
	maxFrames?: number;
	/** Maximum aggregate base64 payload for the rendered archive. If exceeded,
	 *  the oldest visual pages are retired as one chronological prefix. */
	maxFrameDataBytes?: number;
	/** Lazy time-based resolution decay. Re-evaluated whenever compaction runs. */
	memoryDecay?: MemoryDecayOptions;
}

export interface MemoryDecayOptions {
	enabled: boolean;
	periodMs: number;
	scalePerPeriod: number;
	minScale: number;
	/** Deterministic clock override for tests; production omits it. */
	now?: number;
}
...
export function serializeConversation(messages: Message[], options?: SerializeOptions): string {
	...
	const pushPart = (prefix: string, content: string) => {
		...
		if (lastIndex >= 0 && lastPrefix === prefix) {
			const previous = parts[lastIndex] as string;
			const sep = previous.endsWith("\n") || content.startsWith("\n") ? "" : "\n";
			parts[lastIndex] = previous + sep + content;
		} else {
...
interface RawTimedSegment {
	bucketStart: number;
	latestAt: number;
	text: string;
}

function timestampBucket(timestamp: number, periodMs: number): number {
	return Math.floor(timestamp / periodMs) * periodMs;
}

/** Serialize consecutive time buckets independently. Tool results
 * inherit their call's timestamp so a long-running call stays in one bucket. */
function serializeConversationSegments(
	messages: Message[],
	...
	for (const message of messages) {
		...
		for (const block of message.content) {
			if (block.type === "toolCall") callTimes.set(block.id, message.timestamp ?? fallbackTime);
		}
	}
	const groups: Array<{ bucketStart: number; latestAt: number; messages: Message[] }> = [];
	for (const message of messages) {
		const timestamp = message.role === "toolResult"
			? (callTimes.get(message.toolCallId) ?? message.timestamp ?? fallbackTime)
			: (message.timestamp ?? fallbackTime);
		const bucketStart = timestampBucket(timestamp, periodMs);
		const current = groups[groups.length - 1];
		if (current?.bucketStart === bucketStart) {
			current.messages.push(message);
			current.latestAt = Math.max(current.latestAt, timestamp);
		} else {
			groups.push({ bucketStart, latestAt: timestamp, messages: [message] });
		}
	}
	return groups.flatMap(group => {
		const text = serializeConversation(group.messages, options);
		return text.length === 0 ? [] : [{ bucketStart: group.bucketStart, latestAt: group.latestAt, text }];
	});
}
...
function normalizedInputChars(text: string): string[] {
	const stripped = text.includes("\u001b") ? stripVTControlCharacters(text) : text;
...
function nativeRenderOptions(shape: Shape, size: number) {
	return {
		...
		...(shape.stretch === undefined ? {} : { stretch: shape.stretch }),
		variant: shape.variant,
		lineRepeat: shape.lineRepeat,
		...(shape.columns === undefined ? {} : { columns: shape.columns }),
	};
}
...
export function getPreservedArchive(preserveData: Record<string, unknown> | undefined): Archive | undefined {
	...
	const segments = Array.isArray(archive.segments)
		? archive.segments.filter(segment =>
			!!segment &&
			Number.isSafeInteger(segment.bucketStart) &&
			segment.bucketStart >= 0 &&
			Number.isSafeInteger(segment.latestAt) &&
			segment.latestAt >= segment.bucketStart &&
			typeof segment.text === "string" &&
			segment.text.length > 0,
		)
		: [];
	const capturedAt = Number.isSafeInteger(archive.capturedAt) && (archive.capturedAt as number) >= 0
		? archive.capturedAt
		: undefined;
	const informationTokens = Number.isSafeInteger(archive.informationTokens) && (archive.informationTokens as number) > 0
		? archive.informationTokens
		: undefined;
	...
	return {
		...
		...(informationTokens !== undefined ? { informationTokens } : {}),
		...(text !== undefined ? { text } : {}),
...
export function historyBlocks(archive: Archive): (TextContent | ImageContent)[] {
	...
	const archiveImages = images(archive);
	const hasImages = archiveImages.length > 0;
	if (archive.textHead) {
...
interface ArchiveLayout {
	...
	keptText: string;
	truncatedChars: number;
	keptSegments?: TimedArchiveSegment[];
}
...
function mergeTimedSegments(segments: readonly TimedArchiveSegment[]): TimedArchiveSegment[] {
	const merged: TimedArchiveSegment[] = [];
	for (const segment of segments) {
		if (segment.text.length === 0) continue;
		const previous = merged[merged.length - 1];
		if (previous?.bucketStart === segment.bucketStart) {
			previous.text += `${NEWLINE_GLYPH}${segment.text}`;
			previous.latestAt = Math.max(previous.latestAt, segment.latestAt);
		} else {
			merged.push({ ...segment });
		}
	}
...
function joinedTimedText(segments: readonly TimedArchiveSegment[]): string {
	return segments.map(segment => segment.text).join(NEWLINE_GLYPH);
}

function resolutionScaledShape(shape: Shape, scale: number, api: Api | undefined): Shape {
	if (scale >= 1) return shape;
	const family = billingFamily(api);
	const { frameTokenEstimate: _tokens, imageDetail: _detail, ...geometry } = shape;
	// Resolution decay has two jobs: give older glyphs fewer pixels and let a
	// bounded image request carry more old text. Scaling the canvas and cells by
	// the same factor only does the former. A square-root canvas scale keeps the
	// output genuinely lower-resolution while increasing page capacity by about
	// 1/scale, so old buckets consume fewer images as well as fewer visual tokens.
	const frameScale = Math.sqrt(scale);
	return priceShape({
		...geometry,
		frameSize: Math.max(64, Math.round(shape.frameSize * frameScale)),
		cellWidth: Math.max(1, Math.round(shape.cellWidth * scale)),
		cellHeight: Math.max(1, Math.round(shape.cellHeight * scale)),
		// Shrink the glyphs with the frame; never crop a natural-size bitmap font.
		stretch: true,
	}, family);
}

interface DecayedFrameBucket {
	agePeriods: number;
	resolutionScale: number;
...
/** Allocate a small visual working set across time and information volume.
 * Square-root volume gives dense periods more capacity without allowing one
 * huge tool trace to monopolize the archive; resolutionScale makes otherwise
 * equal newer buckets win. Repeated allocation uses diminishing returns, and a
 * partially retained bucket keeps its newest pages. */
function allocateDecayedFrames(
	buckets: readonly DecayedFrameBucket[],
	maxFrames: number,
): { frames: PlanFrame[]; droppedFrames: PlanFrame[] } {
	const nonEmpty = buckets.filter(bucket => bucket.frames.length > 0);
	const allocations = new Map(nonEmpty.map(bucket => [bucket, 0]));
	let remaining = maxFrames;
	while (remaining > 0) {
		let selected: DecayedFrameBucket | undefined;
		let selectedScore = -Infinity;
		for (const bucket of nonEmpty) {
			const allocated = allocations.get(bucket) ?? 0;
			if (allocated >= bucket.frames.length) continue;
			const score = Math.sqrt(bucket.informationChars) * bucket.resolutionScale / (allocated + 1);
	...
	const droppedFrames: PlanFrame[] = [];
	for (const bucket of nonEmpty) {
		const allocated = allocations.get(bucket) ?? 0;
		const split = bucket.frames.length - allocated;
		droppedFrames.push(...bucket.frames.slice(0, split));
		frames.push(...bucket.frames.slice(split));
	}
	return { frames, droppedFrames };
}

interface DecayedSourceBucket {
	segment: TimedArchiveSegment;
	agePeriods: number;
	timeScale: number;
}

function renderDecayedBuckets(
	...
	return buckets.map(bucket => {
		...
		const pages = shape.columns === 2
			? docPages(bucket.segment.text, geo, usesWideCells(shape))
			: paginateCells(bucket.segment.text, geo.capacity, geo.cols, usesWideCells(shape));
		const frames = pages.map(text => ({
			text,
			shape,
			memoryAgePeriods: bucket.agePeriods,
			resolutionScale: scale,
			bucketStart: bucket.segment.bucketStart,
			latestAt: bucket.segment.latestAt,
		}));
		return {
			agePeriods: bucket.agePeriods,
			resolutionScale: scale,
			informationChars: bucket.segment.text.length,
			frames,
		};
	});
}

/** Lay out timestamped history as a lazy sliding window. The complete newest
 * period stays verbatim text. Older periods become progressively denser,
 * lower-resolution frames, while a bounded visual working set is assigned by
 * both source volume and recency. No blur filter is applied. */
function planDecayedArchive(
	segments: readonly TimedArchiveSegment[],
	high: Shape,
	maxFrames: number,
	api: Api | undefined,
	decay: MemoryDecayOptions,
	now: number,
): ArchiveLayout {
	const merged = mergeTimedSegments(segments);
	const sourceBuckets: DecayedSourceBucket[] = [];
	const recent: TimedArchiveSegment[] = [];
	for (const segment of merged) {
		const agePeriods = Math.max(0, Math.floor((now - segment.latestAt) / decay.periodMs));
		if (agePeriods === 0) {
			recent.push({ ...segment });
			continue;
		}
		sourceBuckets.push({
			segment,
			agePeriods,
			timeScale: Math.max(decay.minScale, decay.scalePerPeriod ** agePeriods),
		});
	}
	...
	const frameSegments: TimedArchiveSegment[] = frames.map(frame => ({
		...
		latestAt: frame.latestAt as number,
		text: frame.text,
	}));
	const keptSegments = mergeTimedSegments([...frameSegments, ...recent]);
	return {
		frames,
		textHead: "",
		textTail: joinedTimedText(recent),
		keptText: joinedTimedText(keptSegments),
		truncatedChars: droppedFrames.reduce((sum, frame) => sum + frame.text.length, 0),
		keptSegments,
	};
}
...
interface SummaryPromptValues {
	...
	memoryDecayPeriodHours?: number;
	files?: string;
}

/** Render OMP's snapcompact-summary.md without Bun's text-import loader. */
function renderSummaryPrompt(values: SummaryPromptValues): string {
	const lines = [
	...
	if (values.frameCount > 0) {
		...
		if (values.lineRepeated) lines.push("  - Each line printed twice (white, then pale-yellow band); copies identical.");
		if (values.memoryDecayPeriodHours !== undefined) {
			lines.push(`  - Memory decay: older pages are rendered at progressively lower resolution in ${values.memoryDecayPeriodHours}-hour steps; newest pages are clearest. No blur filter is applied.`);
		}
	}
	...
	lines.push("- If an exact earlier detail matters and a section is unclear, re-derive from workspace (re-read files, re-run commands), rather than guess.", "");
 ...
 * re-renders that source into an ordered history layout. With memory decay it
 * keeps the newest archive edge as text and progressively lowers the resolution
 * of older time buckets; without it, the original text/image/text OMP layout and
 * HQ/LQ/HQ foveated middle are retained.
 *
...
export async function compact<TMessage = Message>(
	...
	const decay = options?.memoryDecay?.enabled === true ? options.memoryDecay : undefined;
	const now = decay?.now ?? Date.now();
	if (decay !== undefined) {
		if (!Number.isSafeInteger(decay.periodMs) || decay.periodMs <= 0) {
			throw new Error(`Invalid memory-decay period: ${decay.periodMs}`);
		}
		if (!Number.isFinite(decay.scalePerPeriod) || decay.scalePerPeriod <= 0 || decay.scalePerPeriod >= 1) {
			throw new Error(`Invalid memory-decay scalePerPeriod: ${decay.scalePerPeriod}`);
		}
		if (!Number.isFinite(decay.minScale) || decay.minScale <= 0 || decay.minScale > 1) {
			throw new Error(`Invalid memory-decay minScale: ${decay.minScale}`);
		}
		if (!Number.isSafeInteger(now) || now < 0) throw new Error(`Invalid memory-decay clock: ${now}`);
	}
	const rawCurrentSegments = decay === undefined
		? []
		: serializeConversationSegments(llmMessages, options, decay.periodMs, now);
	const serialized = decay === undefined
		? serializeConversation(llmMessages, options)
		: rawCurrentSegments.map(segment => segment.text).join(NEWLINE_GLYPH);
	const previousArchive = getPreservedArchive(previousPreserveData);
	...
	const maxFrameDataBytes = options?.maxFrameDataBytes;
	if (maxFrameDataBytes !== undefined && (!Number.isSafeInteger(maxFrameDataBytes) || maxFrameDataBytes <= 0)) {
		throw new Error(`Invalid frame-data budget: ${maxFrameDataBytes}`);
	}

	let truncatedChars = previousArchive?.truncatedChars ?? 0;
	let layout: ArchiveLayout;
	if (decay !== undefined) {
		const fallbackTime = Math.max(0, now - decay.periodMs);
		const previousSegments: TimedArchiveSegment[] = previousArchive?.segments?.map(segment => {
			const healed = elideDataUrls(segment.text, "archive");
			return {
				...segment,
				text: options?.includeThinking === false ? stripThinkingSections(healed) : healed,
			};
		}) ?? (hasPreviousText ? [{
			bucketStart: timestampBucket(previousArchive?.capturedAt ?? fallbackTime, decay.periodMs),
			latestAt: previousArchive?.capturedAt ?? fallbackTime,
			text: previousText,
		}] : []);
		const currentSegments: TimedArchiveSegment[] = rawCurrentSegments.map(segment => ({
			...segment,
			text: elideDataUrls(normalize(segment.text, { shape: high })),
		}));
		const summarySegments: TimedArchiveSegment[] = includedPreviousSummary && previousSummary
			? [{
				bucketStart: timestampBucket(fallbackTime, decay.periodMs),
				latestAt: fallbackTime,
				text: `[Summary of earlier history] ${normalize(previousSummary, { shape: high })}`,
			}]
			: [];
		layout = planDecayedArchive(
			mergeTimedSegments([...summarySegments, ...previousSegments, ...currentSegments]),
			high,
			maxFrames,
			options?.model?.api,
			decay,
	...
	for (const planned of layout.frames) {
		...
		newFrames.push(
			render(pageText, planned.shape).then(rendered => ({
				...
				...(planned.memoryAgePeriods === undefined ? {} : { memoryAgePeriods: planned.memoryAgePeriods }),
				...(planned.resolutionScale === undefined ? {} : { resolutionScale: planned.resolutionScale }),
				...(planned.bucketStart === undefined ? {} : { memoryBucketStart: planned.bucketStart }),
			})),
		);
	}
	...
	let frames = await Promise.all(newFrames);
	// Payload fitting is part of archive planning, not a visibility filter. If
	// the rendered checkpoint is too large, retire one oldest chronological
	// prefix and update the persisted source to exactly match what remains.
	// This avoids a model-visible hole that later masquerades as compression.
	if (maxFrameDataBytes !== undefined && frameDataBytes(frames) > maxFrameDataBytes) {
		let firstKeptFrame = frames.length;
		let keptBytes = 0;
		for (let index = frames.length - 1; index >= 0; index--) {
			const frame = frames[index];
			if (frame === undefined || keptBytes + frame.data.length > maxFrameDataBytes) break;
			keptBytes += frame.data.length;
			firstKeptFrame = index;
		}
		const retiredPlans = layout.frames.slice(0, firstKeptFrame);
		layout.frames = layout.frames.slice(firstKeptFrame);
		frames = frames.slice(firstKeptFrame);
		const retiredChars = retiredPlans.reduce((sum, frame) => sum + frame.text.length, 0);
		layout.truncatedChars += retiredChars;
		truncatedChars += retiredChars;
		if (layout.keptSegments !== undefined) {
			const visualSegments: TimedArchiveSegment[] = layout.frames.map(frame => ({
				bucketStart: frame.bucketStart as number,
				latestAt: frame.latestAt as number,
				text: frame.text,
			}));
			const recentSegments = layout.keptSegments.filter(segment =>
				Math.max(0, Math.floor((now - segment.latestAt) / (decay?.periodMs ?? 1))) === 0,
			);
			layout.keptSegments = mergeTimedSegments([...visualSegments, ...recentSegments]);
			layout.keptText = joinedTimedText(layout.keptSegments);
		} else {
			layout.keptText = `${layout.textHead}${layout.frames.map(frame => frame.text).join("")}${layout.textTail}`;
		}
	}
	...
	if (frames.length === 0 && textHead.length === 0 && textTail.length === 0 && files.length === 0) {
		...
		summary = renderSummaryPrompt({
			...
			...(decay === undefined ? {} : { memoryDecayPeriodHours: decay.periodMs / (60 * 60 * 1000) }),
	...
	const archive: Archive = {
		...
		totalChars,
		truncatedChars,
		informationTokens: Math.max(tokensBefore, previousArchive?.informationTokens ?? 0),
		...(persistedText.length > 0 ? { text: persistedText } : {}),
		...(textHead ? { textHead } : {}),
		...(textTail ? { textTail } : {}),
		...(layout.keptSegments === undefined ? {} : { segments: layout.keptSegments }),
		...(decay === undefined ? {} : { capturedAt: now }),
	};
